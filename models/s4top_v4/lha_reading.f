...
c
      ...
      double precision  alpha, gfermi, alfas,wtt,betat,wm,v
...
c     &        ivalue(n),value(n),name(n)
 ...
 99    continue      
       ...
       call set_it(n,ivalue,value,name,35,bn,h2mass,500d0)
       call set_it(n,ivalue,value,name,36,bn,a2mass,500d0)
       bn="decay"
       ...
       call set_it(n,ivalue,value,name,36,bn,a2width,1d0)
       bn='mguser'
       call set_it(n,ivalue,value,name,1,bn,gch ,1d0)
       call set_it(n,ivalue,value,name,2,bn,gca ,1d0)
       wm = sqrt(zmass**2/Two+
     &     sqrt(zmass**4/Four-Pi/Rt2*alpha/gfermi*zmass**2))
       v   = Two*wm*sqrt( One-(wm/zmass)**2 )/sqrt( alpha * Fourpi )   ! the wmass is used to calculate v
	   if(h2mass.gt.2.0*tmass) then
		 betat=(1.0-4.0*tmass**2.0/h2mass**2.0)**0.5
	     wtt=3.0*mtMS**2.0/v**2.0/8.0/pi
	   else
		 wtt=0.0
	   endif
	   h2width=(wtt*gch**2.0)*h2mass*betat**3.0
	   a2width=(wtt*gca**2.0)*a2mass*betat
      return
      end
